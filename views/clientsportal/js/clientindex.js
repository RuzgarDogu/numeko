...
$(document).ready(function() {
  ...
  var tbl_clientData = $('#tbl-clientData').DataTable( {"dom": 'rtip'} )


  $('#sl-selectClient').select2({
    dropdownParent: $('#mdl-selectClient')
  });

  $('#mdl-selectClient').modal('show').on('shown.bs.modal', function () {
    $.post('clientsportal/getAllClients', function(data, textStatus, xhr) {})
    .done(function(c){
      console.log(c);
      $('#sl-selectClient').html("");
      let opt = '<option value="" selected>Choose Client...</option>';
      c.forEach((item, i) => {
        opt += '<option value="'+item.id+'">'+item.client_name+'</option>'
      });
      $('#sl-selectClient').html(opt);
    });
  })

  $('#btn-selectClient').on('click', function(e) {
    let id = parseInt($('#sl-selectClient').val());
    console.log(id);
    if (id > 0) {
      getClientData(id);
    }
